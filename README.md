...
* waitreaps ([`user/waitreaps.c`](user/waitreaps.c), verified in
  [PROOF.md](PROOF.md)): user-space test that the first wait reaps the
  zombie child exactly once and a second wait returns -1.
...
- sbrksubpage ([`user/sbrksubpage.c`](user/sbrksubpage.c)): user-space test that sbrk(100) moves the break by exactly 100 bytes (break 0x4000 to 0x4064) while the underlying page is fully mapped and usable: a 4096-byte canary over the page reads back byte-exact. 5 checks, 0 mismatches, FNV-1a 0xA8B295C41D629985, byte-identical across 3 QEMU 8.2.2 runs.
